...
def GdbOutCallback(channel: any, text: any)
  ...
  # endif
  # if text =~ '^\^error,msg='
  # text = DecodeMessage(text[11:], false)
  #   if exists('evalexpr') && text =~ 'A syntax error in expression, near\|No symbol .* in current context'
  ...
  # win_gotoid(curwinid)
enddef
##### Check the comm buffer number through :ls and send stuff.

# g:pippo = commbuf
# term_sendkeys(g:pippo, "info breakpoints" .. "\r")


# g:pluto = gdbbuf
# term_sendkeys(g:pluto, "info variables" .. "\r")
